...
ui <- fluidPage(
  ...
  navbarPage("Bioinformatics Software",
             ...
             tabPanel("Assign samples to categories", uiOutput("page3"), 
                      DT::dataTableOutput("page3table"), 
                      actionButton("Lock", "Assign Categories"), 
                      verbatimTextOutput("Button"),
                      verbatimTextOutput("Rows"),
                      verbatimTextOutput("Currentcategory"), 
                      tableOutput("RowsSelected")
...
server <- function(input, output) {
  ...
  currentcategory <- eventReactive(input$Lock, input$selection)
  
  s <- eventReactive(input$Lock, input$page3table_rows_selected)
  
  eventReactive(input$Lock, {selected2[s,'category'] <- currentcategory})
  ...
  output$RowsSelected <- renderTable(selected2)
}
